...
def noise_estimate(df, key, prompt_num):
...
def prompt_analysis(args):
    ...
    for _i, key in enumerate(keywords):
        ...
        if config["PROMPT_ANALYSIS"]["NOISE_ESTIMATE"]:
            biases, noises = noise_estimate(df, key, prompt_num)
            prompt_df["tau"] = biases
            prompt_df["sigma"] = noises
